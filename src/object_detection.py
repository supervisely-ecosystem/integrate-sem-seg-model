...
dst_project_name = f"{src_project_name}_labeled_det"
...
def get_model_classes(class_names):
    ...
    for class_name in class_names:
        model_obj_classes.append(sly.ObjClass(class_name, sly.Rectangle))
...
def apply_model_to_image(image, model, class_names, project_meta):
    ...
    pred_boxes = outputs["instances"].pred_boxes.tensor.detach().numpy()

    # create Supervisely Labels from predicted objects
    labels = []
    for score, class_name, box in zip(pred_scores, pred_class_names, pred_boxes):
        obj_class = project_meta.get_obj_class(class_name)
        conf_tag = sly.Tag(
            meta=project_meta.get_tag_meta("confidence"),
            value=round(float(score), 4),
        )
        geometry = sly.Rectangle(top=box[1], left=box[0], bottom=box[3], right=box[2])
